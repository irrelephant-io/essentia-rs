...
impl From<u32> for HeatCapacity {
    fn from(value: u32) -> Self {
        Self { joule_per_mkelvin: value }
    }
}

impl From<u32> for SpecificHeatCapacity {
    fn from(value: u32) -> Self {
        Self { joule_mol_per_kelvin: value }
    }
}

impl Default for SpecificHeatCapacity {
    fn default() -> Self {
        Self { joule_mol_per_kelvin: 1 }
    }
}
...
impl Add for HeatCapacity {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        Self { joule_per_mkelvin: self.joule_per_mkelvin + rhs.joule_per_mkelvin }
    }
}

impl Sum for HeatCapacity {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        HeatCapacity { joule_per_mkelvin: iter.map(|c| c.joule_per_mkelvin ).sum() }
    }
}