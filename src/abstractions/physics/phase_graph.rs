...
impl PhaseGraphBuilder {
    pub fn add_transition(
        ...
        let left_transition = self.transitions_by_threshold
            .iter()
        ...
        let right_transition = self.transitions_by_threshold
            .iter()
        ...
        if self.transitions_by_threshold.len() > 0 && left_transition.is_none() && right_transition.is_none() {
            panic!("Can't add transitions with gaps!")
        }

        self.transitions_by_threshold.insert(transition.threshold, transition);
