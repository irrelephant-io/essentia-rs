...
impl Default for Dissolution {
    fn default() -> Self {
        Self { optimal_dissolution_speed_percent: 10 }
    }
}
    ...
    fn react(&self, context: &ReactionContext) -> Vec::<Product> {
        ...
        for (solvent, solvent_solubility) in context.engine.iter_solvents() {
            if let Solubility::Solvent(_, saturation_limit) = solvent_solubility {
                ...
                let relative_saturation = solvent_saturation_limit.mmol as f32 / total_saturation_limit.mmol as f32;
...
mod test {
    ...
    fn dissolves_at_minimum_efficiency_if_solution_is_saturated() {
        ...
        if let &Product::Dissolve(eid, fid, _, qty) = products.last().unwrap() {
            ...
            assert_eq!(qty, Quantity::default() * 0.55 * dissolution.optimal_dissolution_speed_percent / 100);
        }
    }
    ...
    fn dissolves_inversly_proportional_to_weight_of_solute() {
        ...
        let products = dissolution.react(&context);
        let sugar_dissolution_qty = products.iter().find_map(|p| {
            match p {
                &Product::Dissolve(essence_id, _, _,qty) if essence_id == Essences::Sugar.into() =>Some(qty),
                _ => None
            }
        }).expect("Expected sugar to start dissolving!");

        let salt_dissolution_qty  = products.iter().find_map(|p| {
            match p {
                &Product::Dissolve(essence_id, _, _, qty) if essence_id == Essences::Salt.into() =>Some(qty),
                _ => None
            }
        ...
        assert!(
            (salt_dissolution_qty.mmol as f32 / sugar_dissolution_qty.mmol as f32) - 2.0 < f32::EPSILON
        )
    }
    ...
    fn dissolves_inversely_proportional_to_saturation() {
        ...
        let products = dissolution.react(&context);
        let aqua_regia_dissolution_qty = products.iter().find_map(|p| {
            match p {
                &Product::Dissolve(_, _, substance_id, qty) if substance_id == aqua_regia_id =>Some(qty),
                _ => None
            }
        }).expect("Expected aqua regia to dissolve salt!");

        let water_dissolution_qty  = products.iter().find_map(|p| {
            match p {
                &Product::Dissolve(_, _, substance_id, qty) if substance_id == water_id => Some(qty),
                _ => None
            }
        ...
        assert!(
            (aqua_regia_dissolution_qty.mmol as f32 / water_dissolution_qty.mmol as f32) - 10.0 < 0.1
        )
    }
}